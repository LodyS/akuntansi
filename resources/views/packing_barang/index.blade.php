...
@section('content')
   <div class="page-header">
     <h1 class="page-title">Packing Barang</h1>
      @include('layouts.inc.breadcrumb')
     <div class="page-header-actions">
     <a class="btn btn-block btn-primary data-modal" id="data-modal" href="#" onclick="show_modal('{{ route('packing-barang.create') }}')" >Tambah</a>
     </div>
   </div>
   <div class="page-content">
    ...
    <button type="button" id="submit" align="right" class="btn btn-primary">Cari</button>
         ...
         <table class="table table-hover dataTable table-striped w-full" id="packing-barang-table">
           <thead>
               <tr>
                 <th>No</th>
                  <th>Persediaan</th>
...
$(function() {
  $('.trash-ck').click(function(){
    if ($('.trash-ck').prop('checked')) {
      document.location = '{{ url("packing-barang?status=trash") }}';
    } else {
      document.location = '{{ url("packing-barang") }}';
    }
  });
    const table = $('#packing-barang-table').DataTable({
    stateSave: true,
    processing : true,
    serverSide : true,
    pageLength:20,
    searching : false,
        ajax : {
                  url:"{{ url('packing-barang/load-data') }}",
                data: function (d) {
                  const form = document.forms.namedItem("formCari");
                    d.barcode = form.barcode.value;
                    d.barang = form.barang.value;
                }
      },
        columns: [
