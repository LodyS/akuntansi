...
<div class="page-header">
    ...
    <div class="page-header-actions">
        <a class="btn btn-block btn-primary data-modal" id="data-modal" href="#"
            onclick="show_modal('{{ route('visit.create') }}')">Tambah</a>
    </div>
</div>
         ...
         <form name="formCari" action="" method="post">
          ...
          <div class="form-group row">
            <label class="col-md-3">No Kunjungan</label>
              <div class="col-md-7">
              <input type="text" name="no_kunjungan" class="form-control">
            </div>
          </div>
          ...
          <button type="button" id="submit" align="right" class="btn btn-primary">Cari</button><br/><br/>
            <table class="table table-hover dataTable table-striped w-full" id="visit-table">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>No. Kunjungan</th>
...
<script type="text/javascript">
  ...
  $('.trash-ck').click(function(){
    if ($('.trash-ck').prop('checked')) {
      document.location = '{{ url("visit?status=trash") }}';
    } else {
      document.location = '{{ url("visit") }}';
    }
  });
    const table = $('#visit-table').DataTable({
      stateSave: true,
    processing : true,
    serverSide : true,
    pageLength:20,
      ...
      language: {
        ...
        aoColumnDefs: [{
                ...
                "render": function ( data, type, row ) {
                    let status = "";
                    switch (data) {
                        case '1': status = "Diperiksa"; break;
                        case '2': status = "Discharge"; break;
                        case '3': status = "Membayar"; break;
                        case '4': status = "Selesai"; break;
                        case '5': status = "Batal"; break;
                    }
                    return status;
                },
            },
