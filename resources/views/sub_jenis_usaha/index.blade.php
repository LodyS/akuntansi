...
@section('content')
   <div class="page-header">
     <h1 class="page-title">Sub Jenis Usaha</h1>
      @include('layouts.inc.breadcrumb')
     <div class="page-header-actions">
     <a class="btn btn-block btn-primary data-modal" id="data-modal" href="#" onclick="show_modal('{{ route('sub-jenis-usaha.create') }}')" >Tambah</a>
     </div>
   </div>
   <div class="page-content">
     ...
     <div class="panel">
       ...
       <div class="panel-body">
        ...
        <!-- </table> -->
         <table class="table table-hover dataTable table-striped w-full" id="sub-jenis-usaha-table">
           <thead>
               <tr>
                 <th>No</th>
                                           <th>kode</th>
...
$(function() {
  $('.trash-ck').click(function(){
    if ($('.trash-ck').prop('checked')) {
      document.location = '{{ url("sub-jenis-usaha?status=trash") }}';
    } else {
      document.location = '{{ url("sub-jenis-usaha") }}';
    }
  });
    const table = $('#sub-jenis-usaha-table').DataTable({
      stateSave: true,
    processing : true,
    serverSide : true,
    pageLength:20,
    searching:false,
        ajax : {
                  url:"{{ url('sub-jenis-usaha/load-data') }}",
                data: function (d) {
...
function con(id,flag_aktif)
{
  if(flag_aktif=='Y')
  {
    var con=confirm("Apakah anda yakin untuk menonaktifkan Sub Jenis Usaha ini?");
    if(con==true)
  {
    window.location.href = "{{ url('sub-jenis-usaha/deactivate/')}}/"+id+"";
  } 
  else
  {
    return false;
  ...
  {
    var con1=confirm(" Anda yakin akan mengaktifkan Sub Jenis Usaha ini?");
    if(con1==true)
  {
     window.location.href = "{{ url('sub-jenis-usaha/activate/')}}/"+id+"";
  } 
  else
  {
    return false;
