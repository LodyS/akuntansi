@extends('layouts.app')

@section('title', $title='Users')
@section('content')
<style>
.btn-primary{
  background-color:#4caf50 ;
  border-color:#4caf50 ;
}
.confirm.btn-primary{
  background-color:#4caf50 ;
  border-color:#4caf50 ;
}
.cancel.btn-default{
  background-color:white ;
  border-color:#4caf50 ;
  color:#4caf50;
}
.btn-primary:hover{
  background-color:#4caf50 ;
  border-color:#4caf50 ;
  color:white;
}
.page-item.active .page-link {
    color: #fff;
    background-color: #4caf50;
    border-color: #4caf50;
}
</style>
   <div class="page-header">
     <h1 class="page-title">Users</h1>
      @include('layouts.inc.breadcrumb')
     <div class="page-header-actions">
     <a class="btn btn-block btn-primary data-modal" id="data-modal" href="#" onclick="show_modal('{{ route('user.create') }}')" >Tambah</a>
     </div>
   </div>
   <div class="page-content">
     <!-- Panel Table Tools -->
     <div class="panel">
       <header class="panel-heading">
         <div class="form-group col-md-12">
           <div class="form-group">
         <!-- <h3 class="panel-title">Table Tools</h3> -->

           </div>
         </div>
       </header>
       <div class="panel-body">
         <!-- <table class="table table-bordered" id="users-table">

         </table> -->
         <table class="table table-hover dataTable table-striped w-full" id="user-table">
           <thead>
               <tr>
                 <th>No</th>
                                 <th>Name</th>
                           <th>Username</th>
                           <th>Email</th>
                       
                           <th>Status Aktif</th>
                           <!-- <th>email_verified_at</th>
                           <th>password</th>
                           <th>remember_token</th>
                           <th>created_at</th> -->
                             <th>Aksi</th>
               </tr>
           </thead>
         </table>
       </div>
     </div>
     <!-- End Panel Table Tools -->
 </div>
 <div class="modal fade" id="formModal" aria-hidden="true" aria-labelledby="formModalLabel" role="dialog" tabindex="-1">
 </div>

@endsection

@push('js')
<script type="text/javascript">
  var user;
$(function() {
	$('.trash-ck').click(function(){
		if ($('.trash-ck').prop('checked')) {
			document.location = '{{ url("user?status=trash") }}';
		} else {
			document.location = '{{ url("user") }}';
		}
	});
    user=$('#user-table').DataTable({
    	stateSave: true,
  		processing : true,
  		serverSide : true,
  		pageLength:20,
      ajax : {
				url:"{{ url('user/load-data') }}",
				data: function (d) {

        }
			},
      columns: [
      	{ data: 'nomor', name: 'nomor',searchable:false,orderable:false },
      	{ data: 'name', name: 'name' },
      	{ data: 'username', name: 'username' },
      	{ data: 'email', name: 'email' },
      	
        
        { data: 'status', name: 'status',searchable:false,orderable:false , "render":function(data,type,row){
          if(data.status_aktif=='Y')
          {
            return '<a class="btn btn-success btn-xs" href="#" style="color:white;font-family:Arial" title="Nonaktifkan User" onclick="con(\'' + data.id + '\',\'' + data.status_aktif + '\')">Aktif</a>';
          }
          else
          {
            return '<a class="btn btn-danger btn-xs" href="#" style="color:white;font-family:Arial" title="Aktifkan User" onclick="con(\'' + data.id + '\',\'' + data.status_aktif + '\')">Nonaktif</a>';
          }
        }},
      	// { data: 'email_verified_at', name: 'email_verified_at' },
      	// { data: 'password', name: 'password' },
      	// { data: 'remember_token', name: 'remember_token' },
      	// { data: 'created_at', name: 'created_at' },
      	{ data: 'action', name: 'action', orderable: false, searchable: false },
    	],
    	language: {
          lengthMenu : '{{ "Menampilkan _MENU_ data" }}',
          zeroRecords : '{{ "Data tidak ditemukan" }}' ,
          info : '{{ "_PAGE_ dari _PAGES_ halaman" }}',
          infoEmpty : '{{ "Data tidak ditemukan" }}',
          infoFiltered : '{{ "(Penyaringan dari _MAX_ data)" }}',
          loadingRecords : '{{ "Memuat data dari server" }}' ,
          processing :    '{{ "Memuat data data" }}',
          sSearchPlaceholder: "Pencarian..",
          lengthMenu: "_MENU_",
          search: "_INPUT_",
          paginate : {
              first :     '{{ "<" }}' ,
              last :      '{{ ">" }}' ,
              next :      '{{ ">>" }}',
              previous :  '{{ "<<" }}'
          }
        },
        aoColumnDefs: [{
          bSortable: false,
          aTargets: [-1]
        }],
        iDisplayLength: 5,
        aLengthMenu: [[5, 10, 25, 50, -1], [5, 10, 25, 50, "All"]],
        // sDom: '<"dt-panelmenu clearfix"Bfr>t<"dt-panelfooter clearfix"ip>',
        // buttons: ['copy', 'excel', 'csv', 'pdf', 'print'],
    });
});

function reload_table()
{
    user.ajax.reload(null,false); //reload datatable ajax 
}

function con(id,status)
{
  // console.log(id);
  // console.log(status);
  
  
 
  // console.log(con);
  if(status=='Y')
  {
    var con=confirm("Apakah anda yakin untuk menonaktifkan user ini?");
    if(con==true)
  {
    window.location.href = "{{ url('user/deactivate/')}}/"+id+"";
  } 
  else
  {
    return false;
  }
  }
  else
  {
    var con1=confirm(" Anda yakin akan mengaktifkan user ini?");
    if(con1==true)
  {
     window.location.href = "{{ url('user/activate/')}}/"+id+"";
  } 
  else
  {
    return false;
  }
  }
  
}

</script>

@endpush
