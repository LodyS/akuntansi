...
@section('content')
   <div class="page-header">
     <h1 class="page-title">Badan Usaha</h1>
      @include('layouts.inc.breadcrumb')
     <div class="page-header-actions">
     <a class="btn btn-block btn-primary data-modal" id="data-modal" href="#" onclick="show_modal('{{ route('jenis-usaha.create') }}')" >Tambah</a>
     </div>
   </div>
   <div class="page-content">
     ...
     <div class="panel">
       ...
       @include('flash-message')
         <form name="formCari" action="" method="post">
                ...
                <div class="form-group row">
                    <label class="col-md-3">Kelompok Bisnis</label>
                      <div class="col-md-7">
                      <select class="form-control" name="kelompok_bisnis">
         ...
         <table class="table table-hover dataTable table-striped w-full" id="jenis-usaha-table">
           <thead>
               <tr>
                 <th>No</th>
                                    <th>Kode</th>
                                    <th>Kelompok Bisnis</th>
                                    <th>Nama</th>
                                    <th>Flag Aktif</th>
                       <th>Edit</th>
               </tr>
...
$(function() {
  $('.trash-ck').click(function(){
    if ($('.trash-ck').prop('checked')) {
      document.location = '{{ url("jenis-usaha?status=trash") }}';
    } else {
      document.location = '{{ url("jenis-usaha") }}';
    }
  });
    const table = $('#jenis-usaha-table').DataTable({
      stateSave: true,
    processing : true,
    serverSide : true,
    searching : false,
    pageLength:20,
        ajax : {
                  url:"{{ url('jenis-usaha/load-data') }}",
                data: function (d) {
                  const form = document.forms.namedItem("formCari");
                  d.nama = form.nama.value;
                  d.kode = form.kode.value;
                  d.kelompok_bisnis = form.kelompok_bisnis.value;
                }
      },
        columns: [
          { data: 'nomor', name: 'nomor',searchable:false,orderable:false },
                                      { data: 'kode', name: 'kode' },
                      { data: 'kelompok_bisnis', name: 'id_kelompok_bisnis' },
                      { data: 'nama', name: 'nama' },
...
function con(id,flag_aktif)
{
  if(flag_aktif=='Y')
  {
    var con=confirm("Apakah anda yakin untuk menonaktifkan Jenis Usaha ini?");
    if(con==true)
  {
    window.location.href = "{{ url('jenis-usaha/deactivate/')}}/"+id+"";
  } 
  else
  {
    return false;
  ...
  {
    var con1=confirm(" Anda yakin akan mengaktifkan Jenis Usaha ini?");
    if(con1==true)
  {
     window.location.href = "{{ url('jenis-usaha/activate/')}}/"+id+"";
  } 
  else
  {
    return false;
