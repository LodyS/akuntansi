...
<div class="page-header">
    <h1 class="page-title">Informasi Setting PL Rekening</h1>
...
<div class="panel-body">
    ...
    <form action="{{ url('informasi-setting-rekening-pl/pencarian')}}" method="POST">{{ @csrf_field() }}

                    <div class="form-group row">
			            <label class="col-md-3">Nama Perkiraan</label>
				            <div class="col-md-7">
				            <select name="nama_perkiraan" id="nama_perkiraan" class="form-control select">
            	                <option value="">Pilih</option>
				                @foreach ($perkiraan as $kira)
				                <option value="{{ $kira->nama }}">{{ $kira->nama }}</option>
				                @endforeach
          		            </select>
			            </div>
		            </div>
                    ...
                    <div class="form-group row">
			            <label class="col-md-3">Kode Rekening</label>
				            ...
				            <select name="kode_rekening" id="kode_rekening" class="form-control select">
            	                <option value="">Pilih</option>
				                @foreach ($perkiraan as $kira)
				                <option value="{{ $kira->kode_rekening }}">{{ $kira->kode_rekening }}</option>
				                @endforeach
          		            </select>
			            </div>
		            </div>
