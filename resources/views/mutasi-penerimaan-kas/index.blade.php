...
<div class="page-content">
    ...
    <div class="panel-body">
           ...
           <form action="{{ url('mutasi-penerimaan-kas/pencarian')  }}" method="post" >{{ @csrf_field() }}
                <div class="form-group row">
                    <label class="col-md-3">Tanggal Awal :</label>
                      	<div class="col-md-7">
                      	<input type="date" name="tanggal_awal" value="{{ $startDate }}" class="form-control" required>
                  	</div>
                </div>
                ...
                <div class="form-group row">
                  	<label class="col-md-3">Tanggal Akhir :</label>
                    	<div class="col-md-7">
                    	<input type="date" name="tanggal_akhir" value="{{ $endDate }}" class="form-control" required>
                  	</div>
                </div>
                ...
                <button type="submit" align="right" class="btn btn-dark">
        			...
        			<table class="table table-hover dataTable table-striped w-full" id="mutasi-kas-table">
                        ...
                        <tr>
                            ...
                            <td class="text-nowrap text-right">Rp. {{ number_format($rekap->nominal,2, ",", ".") }}</td>
                            <td>{{ $rekap->keterangan }}</td>
                            <td>{{ $rekap->status }}</td>
                            <td>{{ ($rekap->flag_bayar == 'Y') ? 'Sudah Dibayar' : 'Belum Dibayar' }}</td>
                            {{-- <td><a href='detail/{{ $rekap->id }}' class='btn btn-primary'><i class="icon glyphicon glyphicon-info-sign" ></i></a>
                            </td> --}}
                            <td class="text-center">
                                @if($rekap->flag_bayar != 'Y')
                                    <button type="button" class="btn btn-sm btn-icon btn-danger btn-round edit"  data-id="{{$rekap->id}}"
                                    data-original-title="Verifikasi" data-toggle="tooltip">
                                    <i class="icon md-check" aria-hidden="true"></i></button>
                                @endif
                            </td>
                            <td class="text-center"> <a class="btn btn-sm btn-info btn-round" href="bukti-transaksi-kas-masuk/{{ $rekap->id }}"
                                data-toggle="tooltip" data-original-title="Bukti Transaksi">
                                <i class="icon glyphicon glyphicon-list-alt" aria-hidden="true"></i></a>
                            </td>
                            <td class="text-center"> <a class="btn btn-sm btn-success btn-round" href="lihat-jurnal-kas-masuk/{{ $rekap->id }}"
...
<div class="modal inmodal fade" id="modal-edit" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-xs">
        <form name="frm_edit" id="frm_edit" class="form-horizontal" action="{{route('verifikasi-mutasi-penerimaan-kas')}}" method="POST">@csrf
            <div class="modal-content">
                <div class="modal-header">
                <h4 class="modal-title">Verifikasi Mutasi Penerimaan Kas : <br/><br/>
...
$('.edit').on("click",function() {
        ...
        //$("#get_id_bank").val(id)
        $.ajax({
            url : "{{route('verif-mutasi-penerimaan-kas')}}?id="+id,
            type: "GET",
            dataType: "JSON",
