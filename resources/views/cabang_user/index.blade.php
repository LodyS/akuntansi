...
@section('content')
   <div class="page-header">
     <h1 class="page-title">Cabang User</h1>
      @include('layouts.inc.breadcrumb')
     <div class="page-header-actions">
     <a class="btn btn-block btn-primary data-modal" id="data-modal" href="#" onclick="show_modal('{{ route('cabang-user.create') }}')" >Tambah</a>
     </div>
   </div>
     ...
     <div class="panel">
       ...
       <div class="panel-body">
        ...
        <button type="button" id="submit" align="right" class="btn btn-primary">Cari</button><br/><br/>
         ...
         <table class="table table-hover dataTable table-striped w-full" id="cabang-user-table">
           <thead>
               <tr>
                 <th>No</th>
...
$(function() {
  $('.trash-ck').click(function(){
    if ($('.trash-ck').prop('checked')) {
      document.location = '{{ url("cabang-user?status=trash") }}';
    } else {
      document.location = '{{ url("cabang-user") }}';
    }
  });
   const table = $('#cabang-user-table').DataTable({
      stateSave: true,
    processing : true,
    serverSide : true,
    searching:false,
