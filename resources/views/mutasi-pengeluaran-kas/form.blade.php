...
<div class="page-content">
    <div class="panel">
		...
		@include('flash-message')
            <h3 align="center">Tambah Mutasi Pengeluaran Kas</h3><br/>
	            <form action="{{ url('/simpan-mutasi-pengeluaran-kas')  }}" method="post">{{ @csrf_field() }}
                    ...
                    <div class="form-group row">
                        <label class="col-md-3">Dibayarkan Kepada</label>
                        <div class="col-md-7">
...
$(document).ready(function() {
    ...
    $('#add').click(function() {
      ...
      $('#dynamic_field').append('<tr id="row' + i + '" class="dynamic-added">\n\
        <td><select class="form-control select " name="id_tarif_pajak[]" id="id_tarif_pajak'+i+'" style="width: 100px;">\n\
        ...
        <td><select class="form-control select " name="id_unit[]">\n\
            <option value="">Pilih Cost Centre</option>\n\
            @foreach ($unit as $u)<option value="{{ $u->id }}">{{ $u->code_cost_centre}} - {{ $u->nama }}</option> @endforeach\n\
            </select>\n\
        </td>\n\
        <td><input type="text" name="keterangan[]" class="form-control name_list " required="" /></td>\n\
        <td><select class="form-control name_list " name="tipe[]" style="width: 120px;">\n\
            <option value="">Pilih</option>\n\
            <option value="1">Penambah</option>\n\
            <option value="-1">Pengurang</option>\n\
            </select></td>\n\
        <td><input type="number" name="nominal[]" class="form-control name_list " required="" style="min-width: 100px;"/>\n\
    ...
    $(document).on('change', 'select[name="id_tarif_pajak[]"]', function(){
        ...
        var url = '{{ route("isiRekening", ":id_tarif_pajak") }}';
